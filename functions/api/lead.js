...
const TOKKO_PROPERTY_ID = 7610290;
const TOKKO_ENDPOINT    = 'https://www.tokkobroker.com/api/v1/webcontact/';
const ALLOWED_ORIGIN    = 'https://cajaravilla.akprop.com.ar';

function corsHeaders(origin) {
  const allowed = origin === ALLOWED_ORIGIN || origin === 'https://cajaravilla-5006.pages.dev';
  return {
...
export async function onRequestPost({ request, env }) {
  ...
  const situacion = (body.situacion || '').trim(); // pre-calificación: cómo compra
  ...
  let text = `Consulta desde landing Cajaravilla 5006. Teléfono/WhatsApp: ${contacto}.`;
  if (situacion) text += ` Situación de compra: ${situacion}.`;
